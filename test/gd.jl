...
# γ-chain plots
p1 = bar(collect(keys(vgenes_G)), collect(values(vgenes_G));
    legend=false, xlabel="V Gene", ylabel="Frequency",
    title="γ-Chain V Gene Frequency")

p2 = bar(collect(keys(jgenes_G)), collect(values(jgenes_G));
    legend=false, xlabel="J Gene", ylabel="Frequency",
    title="γ-Chain J Gene Frequency")

# δ-chain plots
p3 = bar(collect(keys(vgenes_D)), collect(values(vgenes_D));
    legend=false, xlabel="V Gene", ylabel="Frequency",
    title="δ-Chain V Gene Frequency")

p4 = bar(collect(keys(dgenes_D)), collect(values(dgenes_D));
    legend=false, xlabel="D Gene", ylabel="Frequency",
    title="δ-Chain D Gene Frequency")

p5 = bar(collect(keys(jgenes_D)), collect(values(jgenes_D));
    legend=false, xlabel="J Gene", ylabel="Frequency",
...
p1_b = bar(collect(keys(lengths_D)), collect(values(lengths_D));
    legend=false, xlabel="CDR3 Length (in AA)", ylabel="Frequency",
    title="δ-Chain CDR3 Lengths")

p2_b = bar(collect(keys(lengths_G)), collect(values(lengths_G));
...
function center_cdr3s(s::AbstractString, maximum_length::Int; padchar::Char='-')
    ...
    return repeat(string(padchar), padding_left) * s * repeat(string(padchar), padding_right)
end
